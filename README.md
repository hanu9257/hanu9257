# Junepil Lee

Typescript and Javascript developer based on Seoul, Korea🇰🇷

Also using go, python and kotlin for some projects :)

- Interested in generating real tools that can bootstrap users pain point, learning new AI tools, BJJ
- Big fan of git, I had even open [the study](https://github.com/gdsc-konkuk/24-25-git-study) for the newbies in GDGoc Konkuk
- Love learning new languages, I mean real *lauguages*. Available for japanese🇯🇵, spanish🇪🇸, english🇺🇸 and obviously korean🇰🇷

## Experiences
- Konkuk Univ, dept of CSE: 2021.03 - current
- ROKAF Intelligence Information Systems Management Group: 2022.10 - 2024.07
- [GDGoc Konkuk](https://github.com/gdgoc-konkuk): 2024.07 - 2025.07
- University of Western Australia, Exchange Student: 2025.07 - 2025.11
- Musinsa, Personalization Team, Intern: 2026.03 - 2026.09

## Projects
- HR System for goverment workers in ROKAF
- [Chrome extension for room booking in Konkuk University](https://github.com/junepil/kuick)
- [Browser script for bypassing screen time measurement](https://github.com/junepil/lab-safety)
- Creating Graph UI based knowledge management service in [stormer](https://github.com/stormerai)

## Open Source
- Contributed to [@types/threejs](https://github.com/junepil/three-ts-types)
- Currently translating [Hugging Face LLM course](https://github.com/huggingface/course) in korean
